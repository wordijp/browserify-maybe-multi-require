...
var _requires_cache = {};
function _getRequiresFromFiles(browserify, files) {
	browserify
	...
	return _.uniq(_.flatten(files.map(function (file) {
		return _requires_cache[file] = _requires_cache[file] || _.uniq(readRequire(file));
	})));
};
...
function collectModules(browserify, options, cb) {
	...
	mutils.collectRequirePaths(browserify, full_paths, function (files) {
		...
		var requires = _getRequiresFromFiles(browserify, files);
