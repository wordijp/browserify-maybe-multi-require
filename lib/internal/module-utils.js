...
function collectRequirePaths(browserify, paths, cb) {
	...
	var deps = mdeps({
		...
		globalTransform: browserify._transforms
			.filter(function (tr) {
		...
		transform: browserify._transforms
			.filter(function (tr) {
				return tr.transform && !tr.global;
			})
			.map(function (tr) {
	...
	deps.pipe(through.obj(
		...
		function flush(done) {
			...
			for (var file in deps.visited) {
				visited_file_sources[file] = _collect_cache[file] && _collect_cache[file].source || '';
			}
...
function _onUpdateCollectCache(files) {
	files.forEach(function (file) {
		for (var key in _collect_cache) {
			if (putils.isEqualPath(key, file)) {
				delete _collect_cache[key];
				break;
			}
		}
	});
